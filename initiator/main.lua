...
function _M:init_worker()
    ...
    metric_requests_per_second = prometheus:gauge(
    ...
    -- Agregar variables globales para mantener un seguimiento del recuento de solicitudes de las dos últimas segundas
    last_second = 0
    current_second = 0
    request_count_last_second = 0
    request_count_current_second = 0
    
    -- Use el temporizador de la biblioteca resty.stats para contar el número de solicitudes por segundo
    local stats = require "resty.stats"
    local last_time = ngx.now()
    stats.start(1, function()
        local now = ngx.now()
        local delta = now - last_time
        last_time = now
        
        -- Copiar el número actual de solicitudes al recuento del último segundo antes de actualizar el número actual
        request_count_last_second = request_count_current_second
        request_count_current_second = metric_requests:get()
        
        -- Calcular la tasa de solicitudes por segundo entre el recuento del último segundo y el recuento actual
        local rate = (request_count_current_second - request_count_last_second) / delta
        
        -- Actualizar la métrica de tasa de solicitudes por segundo
        metric_requests_per_second:set(rate, {domain})
    end)
end
